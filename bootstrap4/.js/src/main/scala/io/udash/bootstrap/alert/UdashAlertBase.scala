...
private[alert] trait UdashAlertBaseCompanion[T <: UdashAlertBase] {
  ...
  /** Creates an alert with provided style. More: <a
    * href="http://getbootstrap.com/docs/4.1/components/alerts/">Bootstrap Docs</a>.
    *
    * @param alertStyle
    *   A color of the alert.
    * @param componentId
    *   An id of the root DOM node.
    * @param content
    *   A content of the alert. Use the provided interceptor to properly clean up bindings inside the content.
    * @return
    *   A `UdashAlert` component, call `render` to create a DOM element.
    */
  def apply(
