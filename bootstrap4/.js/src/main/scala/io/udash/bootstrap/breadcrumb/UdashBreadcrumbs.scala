...
object UdashBreadcrumbs {
  ...
  val defaultPageFactory: (ReadableProperty[Breadcrumb], Binding.NestedInterceptor) => Modifier = { (page, nested) =>
    nested(produce(page) { page =>
      a(href := page.link)(page.name).render
    })
  }

  /** Creates a breadcrumbs component, synchronised with a provided SeqProperty. More: <a
    * href="http://getbootstrap.com/docs/4.1/components/breadcrumb/">Bootstrap Docs</a>.
    *
    * @param items
    *   The SeqProperty containing breadcrumbs data.
    * @param componentId
    *   An id of the root DOM node.
    * @param itemFactory
    *   Creates a DOM representation for an item from the `items` property. Use the provided interceptor to properly
    *   clean up bindings inside the content.
    * @param isActive
    *   Decides whether an element has an additional `active` style class.
    * @tparam ItemType
    *   A single element's type in the `items` sequence.
    * @tparam ElemType
    *   A type of a property containing an element in the `items` sequence.
    * @return
    *   A `UdashBreadcrumbs` component, call `render` to create a DOM element.
    */
  ...
  /** Creates breadcrumbs component, synchronised with a provided SeqProperty. Based on the default model with a name
    * and a link. More: <a href="http://getbootstrap.com/docs/4.1/components/breadcrumb/">Bootstrap Docs</a>.
    *
    * @param items
    *   The SeqProperty containing breadcrumbs data.
    * @param componentId
    *   An id of the root DOM node.
    * @param itemFactory
    *   Creates a DOM representation for an item from the `items` property.
    * @param isActive
    *   Decides whether an element has an additional `active` style class.
    * @return
    *   A `UdashBreadcrumbs` component, call `render` to create a DOM element.
  ...
  /** Creates breadcrumbs component, synchronised with provided SeqProperty. Based on a simple string elements. More: <a
    * href="http://getbootstrap.com/docs/4.1/components/breadcrumb/">Bootstrap Docs</a>.
    *
    * @param items
    *   The SeqProperty containing breadcrumbs data.
