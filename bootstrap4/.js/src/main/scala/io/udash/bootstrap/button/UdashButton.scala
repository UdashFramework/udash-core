...
import io.udash.wrappers.jquery.{jQ, JQuery}
...
/** Options for `UdashButton` component
  *
  * @param color
  *   A button style, one of the standard bootstrap colors `BootstrapStyles.Color`.
  * @param size
  *   A button size, one of the standard bootstrap sizes `BootstrapStyles.Size`.
  * @param outline
  *   If true, selects the outline style for the button. More: <a
  *   href="http://getbootstrap.com/docs/4.1/components/buttons/#outline-buttons">Bootstrap Docs</a>.
  * @param block
  *   If true, rendered button will be a full-width block.
  * @param tag
  *   HTML tag used in button, one of tags defined in `ButtonTag`
  * @param customModifiers
  *   Sequence of custom modifiers.
  */
...
final class UdashButton private (
  ...
  private val classes: Seq[Modifier] =
    Seq(
      ...
      options.color
        .map(if (options.outline) BootstrapStyles.Button.outline _ else BootstrapStyles.Button.color _): Modifier,
...
object UdashButton {
  ...
  /** Holds button enclosing tag options. Since buttons have their own click listeners implemented they can be enclosed
    ...
    *   - Anchor - encloses the button in <a></a> tags. For this type of tags link options can be used to indicate the
    *     href to redirect user on click
  ...
  /** Creates a button component. More: <a href="http://getbootstrap.com/docs/4.1/components/buttons/">Bootstrap
    * Docs</a>.
    *
    * @param componentId
    *   An id of the root DOM node.
    * @param active
    *   A property indicating if the button is in the `active` state.
    * @param disabled
    *   A property indicating if the button is disabled.
    * @param options
    *   `UdashButton` options object
    * @param content
    *   A content of the button. Use the provided interceptor to properly clean up bindings inside the content.
    * @return
    *   A `UdashButton` component, call `render` to create a DOM element representing this button.
    */
  def apply(
  ...
  /** Creates a toggle button component. More: <a href="http://getbootstrap.com/docs/4.1/components/buttons/">Bootstrap
    * Docs</a>.
    *
    * @param componentId
    *   An id of the root DOM node.
