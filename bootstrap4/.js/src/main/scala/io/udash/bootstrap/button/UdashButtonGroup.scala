...
final class UdashButtonGroup[ItemType, ElemType <: ReadableProperty[ItemType]] private (
  ...
  itemFactory: (ElemType, Binding.NestedInterceptor) => Seq[Element]
...
object UdashButtonGroup {
  ...
  /** Creates a static buttons group. More: <a
    * href="http://getbootstrap.com/docs/4.1/components/button-group/">Bootstrap Docs</a>.
    *
    * @param size
  ...
  def apply(
    ...
    buttons: Element*
  ): UdashButtonGroup[Element, Property[Element]] =
  ...
  /** Creates a dynamic button group. `items` sequence changes will be synchronized with the rendered button group.
    ...
    *   An id of the root DOM node.
    * @param itemFactory
    *   Creates a button based on an item from the `items` sequence. Use the provided interceptor to properly clean up
    *   bindings inside the content.
    * @tparam ItemType
  ...
  /** Creates a dynamic toggle buttons group. `items` sequence changes will be synchronized with the rendered button
    * group. More: <a href="http://getbootstrap.com/docs/4.1/components/button-group/">Bootstrap Docs</a>.
    *
    * @param selectedItems
    ...
    *   An id of the root DOM node.
    * @param btnFactory
    *   It should create UdashButton instance based on provided item and active property. Don't forget to pass the
    *   second argument to created button as `active` arguemnt. The default implementation uses `toString` in order to
    *   create a button's content.
    * @return
    *   A `UdashButtonGroup` component, call `render` to create a DOM element representing this group.
    */
  ...
  /** Creates a dynamic radio buttons group. `items` sequence changes will be synchronized with the rendered button
    * group. More: <a href="http://getbootstrap.com/docs/4.1/components/button-group/">Bootstrap Docs</a>.
    *
    * @param selectedItem
