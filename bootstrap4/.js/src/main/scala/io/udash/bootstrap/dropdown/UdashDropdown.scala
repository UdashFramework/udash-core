  ...
  override lazy val render: Element = {
    ...
    val el = div(
      ...
      nestedInterceptor(buttonFactory { nested =>
        Seq[Modifier](
          nested(BootstrapStyles.Dropdown.toggle.styleIf(buttonToggle)),
          buttonId,
          dataToggle := "dropdown",
          aria.haspopup := true,
          aria.expanded := false,
          buttonContent(nested),
    ...
    nestedInterceptor(
      new JQueryOnBinding(
        jQEl,
        "show.bs.dropdown",
        (_: Element, _: JQueryEvent) => fire(VisibilityChangeEvent(this, EventType.Show)),
      )
    )
    nestedInterceptor(
      new JQueryOnBinding(
        jQEl,
        "shown.bs.dropdown",
        (_: Element, _: JQueryEvent) => fire(VisibilityChangeEvent(this, EventType.Shown)),
      )
    )
    nestedInterceptor(
      new JQueryOnBinding(
        jQEl,
        "hide.bs.dropdown",
        (_: Element, _: JQueryEvent) => fire(VisibilityChangeEvent(this, EventType.Hide)),
      )
    )
    nestedInterceptor(
      new JQueryOnBinding(
        jQEl,
        "hidden.bs.dropdown",
        (_: Element, _: JQueryEvent) => fire(VisibilityChangeEvent(this, EventType.Hidden)),
      )
    )
    el
  }
...
object UdashDropdown {
  ...
  object DropdownEvent {
    ...
    object EventType extends AbstractValueEnumCompanion[EventType] {
      ...
      /** This event is fired when the dropdown has been made visible to the user (will wait for CSS transitions, to
        * complete).
        */
      ...
      /** This event is fired when the dropdown has finished being hidden from the user (will wait for CSS transitions,
        * to complete).
        */
  ...
  /** Creates a dropdown component. More: <a href="http://getbootstrap.com/docs/4.1/components/dropdowns/">Bootstrap
    * Docs</a>.
    *
    * @param items
    ...
    * @param rightAlignMenu
    *   If true, the menu will be aligned to the right side of button.
    * @param buttonToggle
    *   If true, the toggle arrow will be displayed.
    * @param itemFactory
    *   Creates DOM element for each item which is inserted into the dropdown menu. Use the provided interceptor to
    *   properly clean up bindings inside the content. Usually you should add the `BootstrapStyles.Dropdown.item` style
    *   to your element.
    * @param buttonContent
    *   Content of the element opening the dropdown. Use the provided interceptor to properly clean up bindings inside
    *   the content.
    * @param buttonFactory
  ...
  def apply[ItemType, ElemType <: ReadableProperty[ItemType]](
    ...
    val itemBindingFactory = (dropdown: UdashDropdown[ItemType, ElemType]) =>
  ...
  /** Creates a dropdown component with [[DefaultDropdownItem]] as items. More: <a
    * href="http://getbootstrap.com/docs/4.1/components/dropdowns/">Bootstrap Docs</a>.
    *
    * @param items
    *   Data items which will be represented as the elements in this dropdown.
    * @param dropDirection
  ...
  def default[ElemType <: ReadableProperty[DefaultDropdownItem]](
    ...
    val itemBindingFactory: UdashDropdown[DefaultDropdownItem, ElemType] => Binding = dropdown =>
      produceWithNested(items)((items, nested) =>
        items.map(item =>
          withSelectionListener[DefaultDropdownItem, ElemType](defaultItemFactory(item, nested), item, dropdown)
        )
      )
    new UdashDropdown(items, dropDirection, rightAlignMenu, buttonToggle, componentId)(
      itemBindingFactory,
  ...
  private def withSelectionListener[ItemType, ElemType <: ReadableProperty[ItemType]](
    elem: Element,
    item: => ItemType,
    source: UdashDropdown[ItemType, ElemType],
  ): Element = {
    source.nestedInterceptor(
      new source.JQueryOnBinding(
        jQ(elem),
        EventName.click,
        (_: Element, _: JQueryEvent) => source.fire(DropdownEvent.SelectionEvent(source, item)),
      )
    )
    elem
  }
