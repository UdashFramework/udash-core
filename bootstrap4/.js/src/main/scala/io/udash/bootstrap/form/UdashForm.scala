...
object UdashForm {
  ...
  object ValidationTrigger extends AbstractValueEnumCompanion[ValidationTrigger] {
    ...
    /** The validation will be triggered on the form `submit` event. Notice that the validation won't block the form
      * submit.
      */
    final val OnSubmit: Value = new ValidationTrigger
  }

  /** Settings for the horizontal form layout. More: <a
    * href="http://getbootstrap.com/docs/4.1/components/forms/#horizontal-form">Bootstrap Docs</a>.
    *
    * @param labelWidth
  ...
  /** Creates a standard form with a provided content. <br/> Example: <br/> <pre> UdashForm() { factory => Seq(
    ...
    * validFeedback = _ => Some(span("Looks good.")), invalidFeedback = _ => Some(span("Name is too short.")) )
    * )}.render </pre> More: <a href="http://getbootstrap.com/docs/4.1/components/forms/">Bootstrap Docs</a>.
    *
    * @param inline
    *   If true, creates an inline form.
    * @param inputValidationTrigger
    *   Default validation trigger for text inputs in this form.
    * @param selectValidationTrigger
...
final class FormElementsFactory(
  ...
  /** Use this method to bond the external binding's lifecycle with the lifecycle of the elements created via this
    * factory.
    */
  ...
  /** Wrapper for disabled elements.
    ...
    */
  def disabled(
    disabled: ReadableProperty[Boolean] = UdashBootstrap.True
  )(
  ...
  object input {
    ...
    /** Creates a standard form group for the provided input. It contains a label and validation feedback elements. The
      * group layout can be organized vertically or horizontally based on the provided options. You can wrap the result
      * into grid elements in order to create more complex form layout.
      *
      * @param horizontal
      ...
      * @param input
      *   The input element. It can be wrapped into `UdashInputGroup` or any other decoration. Use the provided
      *   interceptor to properly clean up bindings inside the content.
      * @param labelContent
      *   Optional label content. It will be wrapped into `label` element with properly set `for` attribute. Use the
      *   provided interceptor to properly clean up bindings inside the content.
      * @param validFeedback
      *   Optional content of positive validation feedback. It will be wrapped into `div` element with `valid-feedback`
      *   style. Use the provided interceptor to properly clean up bindings inside the content.
      * @param invalidFeedback
      *   Optional content of negative validation feedback. It will be wrapped into `div` element with
      *   `invalid-feedback` style. Use the provided interceptor to properly clean up bindings inside the content.
      * @param helpText
      *   Optional content of help text block. It will be wrapped into `div` element with `form-text text-muted` style.
        ...
        override val render: Element = horizontal match {
          case None =>
            ...
            div(BootstrapStyles.Form.group, groupId)(
              ...
              helpText.map(content =>
                div(BootstrapStyles.Form.text, BootstrapStyles.Text.muted)(content(nestedInterceptor))
              ),
            ).render
    ...
    /** Creates a text input with a default bootstrap styling and an optional validation callback which sets proper
      * bootstrap classes: `is-valid` and `is-invalid`. Use `formGroup` if you want to create an input with a label and
      ...
      * @param size
      *   Size of the input.
      * @param validationTrigger
      *   Selects the event updating validation state of the input.
      * @param inputId
      *   Id of the input DOM element.
      * @param inputModifier
      *   Modifiers applied directly to the `input` element. Use the provided interceptor to properly clean up bindings
      *   inside the content.
      * @param validator
      *   Validator for provided text property
      */
    def textInput(
    ...
    /** Creates a password input with a default bootstrap styling and an optional validation callback which sets proper
      * bootstrap classes: `is-valid` and `is-invalid`. Use `formGroup` if you want to create an input with a label and
      * validation feedback elements.
      *
      * @param property
      ...
      *   inside the content.
      * @param validator
      *   Validator for provided password property
      */
    def passwordInput(
      property: Property[String],
      debounce: Duration = 20.millis,
    ...
    /** Creates a number input with a default bootstrap styling and an optional validation callback which sets proper
      * bootstrap classes: `is-valid` and `is-invalid`. Use `formGroup` if you want to create an input with a label and
      * validation feedback elements.
      *
      * @param property
      ...
      *   inside the content.
      * @param validator
      *   Validator for provided number property
      */
    def numberInput(
    ...
    /** Creates a range input with a custom bootstrap styling and an optional validation callback which sets proper
      * bootstrap classes: `is-valid` and `is-invalid`. Use `formGroup` if you want to create an input with a label and
      * validation feedback elements.
      *
      * @param value
      *   Current value synchronised with the input. The value should be between `minValue` and `maxValue`. It should be
      *   also divisible by `valueStep`.
      * @param minValue
      ...
      *   inside the content.
      * @param validator
      *   Validator for provided value property
      */
    def rangeInput(
    ...
    /** Creates a text area with a default bootstrap styling and an optional validation callback which sets proper
      * bootstrap classes: `is-valid` and `is-invalid`. Use `formGroup` if you want to create an input with a label and
      * validation feedback elements.
      *
      * @param property
    ...
    /** Creates a file input with a default bootstrap styling and an optional validation callback which sets proper
      * bootstrap classes: `is-valid` and `is-invalid`. It creates the whole input compoennt including a label and
      * validation feedback.
      *
      * @param selectedFiles
      *   This property contains information about files selected by a user.
      * @param acceptMultipleFiles
      ...
      *   inside the content.
      * @param labelContent
      *   Required label content. It will be wrapped into `label` element with properly set `for` attribute. Use the
      *   provided interceptor to properly clean up bindings inside the content.
      * @param validFeedback
      *   Optional content of positive validation feedback. It will be wrapped into `div` element with `valid-feedback`
      *   style. Use the provided interceptor to properly clean up bindings inside the content.
      ...
      *   `invalid-feedback` style. Use the provided interceptor to properly clean up bindings inside the content.
      * @param validator
      *   Validator for provided files property
      */
    def fileInput(
    ...
    /** Creates a select menu with a default bootstrap styling and an optional validation callback which sets proper
      * bootstrap classes: `is-valid` and `is-invalid`. Use `formGroup` if you want to create an input with a label and
      * validation feedback elements.
      *
      * @param selectedItem
      ...
      *   inside the content.
      * @param validator
      *   Validator for provided item property
      */
    def select[T](
    ...
    /** Creates a multiple select menu with a default bootstrap styling and an optional validation callback which sets
      * proper bootstrap classes: `is-valid` and `is-invalid`. Use `formGroup` if you want to create an input with a
      * label and validation feedback elements.
      *
      * @param selectedItems
      ...
      *   inside the content.
      * @param validator
      *   Validator for provided items property
      */
    ...
    /** Creates a checkbox with a custom bootstrap styling and an optional validation callback which sets proper
      * bootstrap classes: `is-valid` and `is-invalid`. It also creates a label and validation feedback elements for the
      * checkbox.
      *
      * @param property
      ...
      *   `invalid-feedback` style. Use the provided interceptor to properly clean up bindings inside the content.
      * @param validator
      *   Validator for provided checkbox state property
      */
    def checkbox(
      ...
      externalBinding(new UdashBootstrapComponent {
        private val input = nestedInterceptor(
          Checkbox(property)(
            inputId,
            BootstrapStyles.Form.control,
            inputModifier.map(_.apply(nestedInterceptor)),
        ...
        override val render: Element = div(
          groupId,
          BootstrapStyles.Form.customControl,
          BootstrapStyles.Form.customCheckbox,
    ...
    /** Creates a checkboxes with a custom bootstrap styling and an optional validation callback which sets proper
      * bootstrap classes: `is-valid` and `is-invalid`. It also creates a label and validation feedback elements for
      * each checkbox.
      *
      * @param selectedItems
      *   Property which will be synchronised with the checkbox state and validated.
      ...
      *   Id of the root element.
      * @param inputModifier
      *   Modifiers applied directly to the `input` element. The factory takes item value and index as the arguments.
      *   Use the provided interceptor to properly clean up bindings inside the content.
      * @param labelContent
      *   Optional label content. It will be wrapped into `label` element with properly set `for` attribute. The factory
      *   takes item value and index as the arguments. Use the provided interceptor to properly clean up bindings inside
      *   the content.
      * @param validFeedback
      *   Optional content of positive validation feedback. It will be wrapped into `div` element with `valid-feedback`
      *   style. The factory takes item value and index as the arguments. Use the provided interceptor to properly clean
      *   up bindings inside the content.
      * @param invalidFeedback
      *   Optional content of negative validation feedback. It will be wrapped into `div` element with
      *   `invalid-feedback` style. The factory takes item value and index as the arguments. Use the provided
      *   interceptor to properly clean up bindings inside the content.
      * @param validator
      *   Validator for provided checkbox states property
      */
    def checkButtons[T](
      ...
      externalBinding(
        new ButtonsComponent(
          selectedItems,
          CheckButtons(selectedItems, options)(_: Seq[(JSInput, T)] => Seq[Node]),
          BootstrapStyles.Form.customCheckbox,
          inline,
          validationTrigger,
          groupId,
    ...
    /** Creates radio buttons with a custom bootstrap styling and an optional validation callback which sets proper
      * bootstrap classes: `is-valid` and `is-invalid`. It also creates a label and validation feedback elements for
      * each radio button.
      *
      * @param selectedItem
      ...
      *   interceptor to properly clean up bindings inside the content.
      * @param validator
      *   Validator for provided item selection property
      */
    def radioButtons[T](
      selectedItem: Property[T],
      options: ReadableSeqProperty[T],
      ...
      externalBinding(
        new ButtonsComponent(
          selectedItem,
          RadioButtons(selectedItem, options)(_: Seq[(JSInput, T)] => Seq[Node]),
          BootstrapStyles.Form.customRadio,
          inline,
          validationTrigger,
          groupId,
        )(inputModifier, labelContent, validFeedback, invalidFeedback, validator)
      ...
      private val inputs = nestedInterceptor(input { inputs =>
        ...
        inputs.zipWithIndex.map { case ((singleInput, item), idx) =>
          ...
          div(
            singleInput,
            BootstrapStyles.Form.customControl,
            inputDecorationClass,
            nestedInterceptor(BootstrapStyles.Form.customControlInline.styleIf(inline)),
            labelContent(item, idx, nestedInterceptor).map(
              label(`for` := singleInput.id, BootstrapStyles.Form.customControlLabel)(_)
            ),
    ...
    private def validationModifier[ArgumentType](
      property: ReadableProperty[ArgumentType],
      validationTrigger: ValidationTrigger,
      ...
      validator: Validator[ArgumentType]
    ): Modifier = {
        ...
        case ValidationTrigger.Instant =>
          ...
          Seq(
            nested(new Binding {
              override def applyTo(t: Element): Unit = {
                propertyListeners +=
                  property.listen(_ => startValidation(validationResult, triggerGroup = true), initUpdate = true)
                form.foreach(_.validationProperties += validationResult)
              }
              override def kill(): Unit = {
                super.kill()
            ...
            nested(BootstrapStyles.Form.isValid.styleIf(validationResult.transform(_.contains(Valid)))),
            nested(
              BootstrapStyles.Form.isInvalid.styleIf(validationResult.transform(v => v.isDefined && !v.contains(Valid)))
            ),
          )
...
object FormElementsFactory {
  def apply(
    ...
    new FormElementsFactory(nestedInterceptor, inputValidationTrigger, selectValidationTrigger, form)
}
