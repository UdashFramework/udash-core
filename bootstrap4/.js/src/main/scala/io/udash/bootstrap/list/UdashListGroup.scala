...
final class UdashListGroup[ItemType, ElemType <: ReadableProperty[ItemType]] private (
  ...
  override val render: Element =
    div(
      componentId,
      BootstrapStyles.ListGroup.listGroup,
...
object UdashListGroup {
  ...
  /** Creates a list group component, synchronised with a provided items sequence. More: <a
    * href="http://getbootstrap.com/docs/4.1/components/list-group/">Bootstrap Docs</a>.
    *
    * @param items
    *   Data items which will be represented as DOM elements in this group.
    * @param flush
    *   If true, applies `list-group-flush` style.
    * @param componentId
    *   An id of the root DOM node.
    * @param itemFactory
    *   Creates DOM representation of the provided element. Use the provided interceptor to properly clean up bindings
    *   inside the content.
    * @tparam ItemType
  ...
  def apply[ItemType, ElemType <: ReadableProperty[ItemType]](
    ...
    itemFactory: (ElemType, Binding.NestedInterceptor) => Element
  ): UdashListGroup[ItemType, ElemType] =
