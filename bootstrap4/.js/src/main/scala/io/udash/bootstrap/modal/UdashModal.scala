...
final class UdashModal private (
  ...
  /** Readjusts the modal's positioning to counter a scrollbar in case one should appear, which would make the modal
    * jump to the left.
    */
  ...
  override val render: Element = {
    ...
    nestedInterceptor(
      new JQueryOnBinding(
        jQEl,
        "show.bs.modal",
        (_: Element, _: JQueryEvent) => fire(ModalEvent(this, ModalEvent.EventType.Show)),
      )
    )
    nestedInterceptor(
      new JQueryOnBinding(
        jQEl,
        "shown.bs.modal",
        (_: Element, _: JQueryEvent) => fire(ModalEvent(this, ModalEvent.EventType.Shown)),
      )
    )
    nestedInterceptor(
      new JQueryOnBinding(
        jQEl,
        "hide.bs.modal",
        (_: Element, _: JQueryEvent) => fire(ModalEvent(this, ModalEvent.EventType.Hide)),
      )
    )
    nestedInterceptor(
      new JQueryOnBinding(
        jQEl,
        "hidden.bs.modal",
        (_: Element, _: JQueryEvent) => fire(ModalEvent(this, ModalEvent.EventType.Hidden)),
      )
    )
    el
  }
...
object UdashModal {
  ...
  object ModalEvent {
    ...
    object EventType extends AbstractValueEnumCompanion[EventType] {
      ...
      /** This event fires immediately when the show instance method is called. If caused by a click, the clicked
      ...
      /** This event is fired when the modal has been made visible to the user (will wait for CSS transitions to
        * complete). If caused by a click, the clicked element is available as the relatedTarget property of the event.
        */
  ...
  /** Creates a modal window. More: <a href="http://getbootstrap.com/docs/4.1/components/modal/">Bootstrap Docs</a>.
    *
    * @param modalSize
    *   A window size. One of the standard bootstrap sizes `BootstrapStyles.Size`.
    * @param fade
    *   If true, show and hide will be animated.
    * @param labelId
    *   An id of the label describing this modal.
    * @param backdrop
    *   Modal backdrop type.
    * @param keyboard
    *   If true, allows user to close modal with keyboard (Esc button).
    * @param componentId
    *   An id of the root DOM node.
    * @param headerFactory
    *   Creates content of modal header. Modal will be rendered without the header if `None`. Use the provided
    *   interceptor to properly clean up bindings inside the content.
    * @param bodyFactory
    *   Creates content of modal body. Modal will be rendered without body if `None`. Use the provided interceptor to
    *   properly clean up bindings inside the content.
    * @param footerFactory
    *   Creates content of modal footer. Modal without footer will be rendered if `None`. Use the provided interceptor
    *   to properly clean up bindings inside the content.
    * @return
    *   A `UdashModal` component, call `render` to create a DOM element.
    */
  def apply(
  ...
  /** Attributes which should be added to button closing the modal window. Example:
    * `UdashButton()(_ => Seq[Modifier](UdashModal.CloseButtonAttr, "Close..."))`
    */
