...
final class UdashProgressBar private[progressbar] (
  ...
  labelFactory: (
    ReadableProperty[Int],
    ReadableProperty[Int],
    ReadableProperty[Int],
    Binding.NestedInterceptor,
  ) => Modifier
  ...
  protected def barModifiers: Seq[Modifier] = Seq[Modifier](
    ...
    nestedInterceptor(
      width.bind(
        progress.combine(minValue.combine(maxValue)((_, _)))((_, _)).transform { case (current, (min, max)) =>
          s"${100 * (current - min) / (max - min)}%"
        }
      )
    ),
...
object UdashProgressBar {
  val ToStringValueStringifier
    : (ReadableProperty[Int], ReadableProperty[Int], ReadableProperty[Int], Binding.NestedInterceptor) => Modifier =
    (p, _, _, nested) => nested(bind(p.transform(_.toString)))

  /** Default method of converting progress to string. */
  val PercentValueStringifier
    : (ReadableProperty[Int], ReadableProperty[Int], ReadableProperty[Int], Binding.NestedInterceptor) => Modifier = {
    (progress, min, max, nested) =>
      nested(
        bind(
          progress.combine(min.combine(max)((_, _)))((_, _)).transform { case (current, (min, max)) =>
  ...
  /** Creates a progress bar component. More: <a href="http://getbootstrap.com/docs/4.1/components/progress/">Bootstrap
    * Docs</a>.
    *
    * @param progress
    *   Property containing Integer in range from `minValue` to `maxValue`.
    * @param showPercentage
    *   If true, display progress string.
    * @param barStyle
    *   A bar color. One of the standard bootstrap colors `BootstrapStyles.Color`.
    * @param stripped
    *   If true, applies striped style to the bar.
    * @param animated
    *   If true, applies animation style to the bar.
    * @param minValue
    *   A minimum progress value.
    * @param maxValue
    *   A maximum progress value.
    * @param minWidth
    *   A minimal width of the progress indicator.
    * @param componentId
    *   An id of the root DOM node.
    * @param labelFactory
    *   Creates a label content from current progress, min value, max value. Use the provided interceptor to properly
    *   clean up bindings inside the content.
    * @return
    *   A `UdashProgressBar` component, call `render` to create a DOM element.
    */
  def apply(
    progress: ReadableProperty[Int],
    ...
    labelFactory: (
      ...
      ReadableProperty[Int],
      Binding.NestedInterceptor,
    ) => Modifier = PercentValueStringifier
  ): UdashProgressBar =
