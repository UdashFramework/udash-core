...
object UdashTable {
  ...
  /** Creates a table component. More: <a href="http://getbootstrap.com/docs/4.1/content/tables/">Bootstrap Docs</a>.
    *
    * @param items
    ...
    * @param rowFactory
    *   Creates row representation of the table element - it should create the `tr` tag. Use the provided interceptor to
    *   properly clean up bindings inside the content.
    * @param headerFactory
    *   Creates table header - it should create the `tr` tag. Table without header will be rendered if `None` passed.
    *   Use the provided interceptor to properly clean up bindings inside the content.
    * @param captionFactory
    *   Creates table caption - the result will be wrapped into the `caption` tag. Table without caption will be
    *   rendered if `None` passed. Use the provided interceptor to properly clean up bindings inside the content.
    * @tparam ItemType
