...
trait Bindings {
  ...
  /** Use it to bind value of property into DOM structure. Value of the property will be rendered as text node. (Using
    * .toString method.) If property value is null, empty text node will be added.
    *
    * @param property
    *   `Property` to bind.
    * @return
  ...
  /** Shows provided DOM elements only if property value is `true`.
    ...
    *   `Property` to check.
    * @param elements
    *   `Element`s to show if property value is `true`.
    * @return
    *   property binding.
    */
  ...
  /** Shows provided DOM elements only if property value is `true`.
    ...
    * @param elements
    *   `Element`s to show if property value is `true`.
    * @param customElementsReplace
    *   takes root element, old children and new children. It should return `true`, if it did not replace elements in
    *   DOM. Is such a case the default implementation will replace the elements. Otherwise you have to replace elements
    *   in DOM manually.
    * @return
    *   property binding.
    */
  def showIf(
    property: ReadableProperty[Boolean],
  ...
  /** Switches provided DOM elements depending on property value.
    ...
    *   `Element`s to show if property value is `true`.
    * @param elseElements
    *   `Element`s to show if property value is `false`.
    * @return
    *   property binding.
    */
  ...
  def showIfElse(
    ...
    elements: => Seq[Node],
    elseElements: => Seq[Node],
  ): Binding =
    produce(property, customElementsReplace, checkNull = true)(if (_) elements else elseElements)

  /** Use it to bind property into DOM structure, given `builder` will be used to generate DOM element on every value
    * change. If property value is null, empty text node will be added as placeholder.
    *
    * @param property
    *   `Property` to bind.
    * @param builder
    ...
    * @param checkNull
    *   if it is true, then null value of property will result in rendering empty text node. if it is false, then null
    *   value has to be handled by builder.
    * @return
    *   property binding.
    */
  ...
  /** Use it to bind property into DOM structure, given `builder` will be used to generate DOM element on every value
    * change. If property value is null, empty text node will be added as placeholder.<br/><br/>
    *
    ...
    *   property binding.
    */
  def produceWithNested[T](
    property: ReadableProperty[T],
    checkNull: Boolean = true,
  )(
    builder: (T, Binding.NestedInterceptor) => Seq[Node]
  ): Binding =
  ...
  /** Use it to bind sequence property into DOM structure, given `builder` will be used to generate DOM element on every
    * value change. Notice that on every property change, whole element representing property will be rendered again.
    *
    * @param property
    *   `Property` to bind.
    * @param builder
  ...
  /** Use it to bind sequence property into DOM structure, given `builder` will be used to generate DOM element on every
    * value change. Notice that on every property change, whole element representing property will be rendered
    * again.<br/><br/>
    *
    * The builder takes nested bindings interceptor - it should be used if you want to create another binding inside
    * this builder. This prevents memory leaks by killing nested bindings on property change. <br/><br/>
  ...
  def produceWithNested[T](
    ...
    builder: (Seq[T], Binding.NestedInterceptor) => Seq[Node]
  ): Binding =
  ...
  /** Use it to bind sequence property into DOM structure. This method cares about adding new elements which appears in
    * sequence and also removes those which were removed. You only need to provide builder which is used to create HTML
    * element for each sequence member. <br/> <b>Note:</b> This will handle only structure changes, if you want to
    * handle concrete subproperties changes, you should use another binding method inside `builder`.
    *
    * @param property
    *   `Property` to bind.
    * @param builder
    *   `Builder` which is used for every element.
    * @param customElementsInsert
    *   takes root element, ref node and new children. It should return `true`, if it does not insert elements in DOM.
    *   Is such a case the default implementation will insert the elements. Otherwise you have to insert elements in DOM
    *   manually.
    * @param customElementsReplace
    *   takes root element, old children and new children. It should return `true`, if it did not replace elements in
    *   DOM. Is such a case the default implementation will replace the elements. Otherwise you have to replace elements
    ...
    *   property binding.
    */
  def repeat[T, E <: ReadableProperty[T]](
  ...
  /** Use it to bind sequence property into DOM structure. This method cares about adding new elements which appears in
    * sequence and also removes those which were removed. You only need to provide builder which is used to create HTML
    * element for each sequence member. <br/> <b>Note:</b> This will handle only structure changes, if you want to
    * handle concrete subproperties changes, you should use another binding method inside `builder`.<br/><br/>
    *
    * The builder takes nested bindings interceptor - it should be used if you want to create another binding inside
    * this builder. This prevents memory leaks by killing nested bindings on property change. <br/><br/>
    *
    ...
    *   property binding.
    */
  def repeatWithNested[T, E <: ReadableProperty[T]](
    property: ReadableSeqProperty[T, E],
    customElementsReplace: DOMManipulator.ReplaceMethod = DOMManipulator.DefaultElementReplace,
    customElementsInsert: DOMManipulator.InsertMethod = DOMManipulator.DefaultElementInsert,
  )(
  ...
  /** Use it to bind sequence property into DOM structure. This method cares about adding new elements which appears in
    * sequence and also removes those which were removed. You only need to provide builder which is used to create HTML
    * element for each sequence member. This modifier provides also property with element index in sequence. <br/>
    * <b>Note:</b> This will handle only structure changes, if you want to handle concrete subproperties changes, you
    * should use another binding method inside `builder`.<br/><br/>
    *
    * The builder takes nested bindings interceptor - it should be used if you want to create another binding inside
    * this builder. This prevents memory leaks by killing nested bindings on property change. <br/><br/>
    *
    ...
    *   property binding.
    */
  def repeatWithIndex[T, E <: ReadableProperty[T]](
    property: ReadableSeqProperty[T, E],
    customElementsReplace: DOMManipulator.ReplaceMethod = DOMManipulator.DefaultElementReplace,
    customElementsInsert: DOMManipulator.InsertMethod = DOMManipulator.DefaultElementInsert,
  )(
  ...
  implicit final class InlineStyleOps[T: HasCssName](style: T) {
    ...
    /** Synchronises style value with property content by adding it when property is not null and condition property is
      * 'true' and removing otherwise.
      */
...
object Bindings extends Bindings {
  ...
  final class AttrOps(private val attr: Attr) extends AnyVal {
    ...
    /** Use this to add more events listeners to an attribute (:= always overrides previous binding). If callback
      * returns true, other listeners which are queued will not be invoked. If callback returns false, next callback in
      * the queue will be invoked. Results other than booleans are treated as false - they don't prevent event
      * propagation.
      */
    def :+=[T <: Event](callback: T => Any): Modifier[Element] =
      AttrPair(
        attr,
        callback,
        (el: Element, attr: Attr, callback: T => Any) => {
          val dyn: js.Dynamic = el.asInstanceOf[js.Dynamic]
          val existingCallbacks: js.Function1[T, Boolean] =
            dyn.selectDynamic(attr.name).asInstanceOf[js.Function1[T, Boolean]]
    ...
    /** Synchronises attribute value with property content by adding it when property is not null and condition property
      * is 'true' and removing otherwise.
      */
    def bindIf(property: ReadableProperty[String], conditionProperty: ReadableProperty[Boolean]): Binding =
      property.combine(conditionProperty)((_, _)).reactiveApply {
  ...
  final class AttrPairOps(private val attr: scalatags.generic.AttrPair[Element, _]) extends AnyVal {
    ...
    def attrIfNot(property: ReadableProperty[Boolean]): Binding =
      property.reactiveApply((elem, apply) =>
