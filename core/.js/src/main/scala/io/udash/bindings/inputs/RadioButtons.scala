...
object RadioButtons {
  ...
  /** @param selectedItem
    *   Property which is going to be bound to radio buttons group.
    * @param options
    *   Seq of available options, one radio button will be created for each option.
    * @param decorator
    *   Function creating HTML element from buttons Seq. (Check: `RadioButtons.inputsOnlyDecorator`,
    *   `RadioButtons.spanWithLabelDecorator` and `RadioButtons.divWithLabelDecorator`)
