...
object RangeInput {
  ...
  /** @param property
    ...
    *   Additional Modifiers, don't use modifiers on value, onchange, min, max and step attributes.
    * @return
    *   HTML range input with bound Property, applied modifiers and nested options.
    */
  def apply(
    ...
    new InputBinding[JSInput] {
      ...
      propertyListeners += minValue.listen(
        { v =>
      ...
      propertyListeners += maxValue.listen(
        { v =>
      ...
      propertyListeners += valueStep.listen(
        { v =>
