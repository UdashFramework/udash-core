...
object Select {
  ...
  /** Single select for ValueProperty.
    ...
    * @param label
    *   Provides element's label.
    * @param selectModifiers
    *   Additional Modifiers for the select tag, don't use modifiers on value, onchange and selected attributes.
    * @return
    *   Binding with `select` element, which can be used as Scalatags modifier.
    */
  def apply[T](
  ...
  /** Single select for optional ValueProperty. It differs from the `apply` method since the rendered output contains
    * the empty string for the empty value. It might be useful when you need to use the `required` attribute
    ...
    *   SeqProperty of available options.
    * @param labelNoValue
    *   Label for option without value
    * @param label
    *   Provides element's label.
    * @param selectModifiers
    *   Additional Modifiers for the select tag, don't use modifiers on value, onchange and selected attributes.
    ...
    */
  def optional[T](
    selectedItem: Property[Option[T]],
    options: ReadableSeqProperty[T],
    labelNoValue: Modifier,
  )(
    label: T => Modifier,
    selectModifiers: Modifier*
  ): InputBinding[Select] =
  ...
  /** Multi selection for SeqProperty. Bound SeqProperty will contain selected options.
    ...
    *   SeqProperty of available options.
    * @param label
    *   Provides element label.
    * @param selectModifiers
    *   Additional Modifiers, don't use modifiers on value, onchange and selected attributes.
    * @return
    *   Binding with `select` element, which can be used as Scalatags modifier.
    */
  def apply[T, ElemType <: Property[T]](
    selectedItems: SeqProperty[T, ElemType],
    options: ReadableSeqProperty[T],
  )(
    label: T => Modifier,
    selectModifiers: Modifier*
