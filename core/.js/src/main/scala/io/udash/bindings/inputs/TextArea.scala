...
object TextArea {
  ...
  /** @param value
    ...
    * @param onInputElementEvent
    *   Callback that's executed when `Input` element receives one of following events: `Input`, `Change`, `KeyUp`,
    *   `Paste` and element value is different than property value. Can Can be used to build unidirectional data flow
    *   component on top of `TextArea` component.
    * @param textareaModifiers
    *   Additional modifiers. Attributes: `tpe`, `value`, `onkeyup`, `onchange`, `onpaste`, `oninput` are ignored as
    *   they are overwritten internally.
    * @return
    *   HTML textarea with bound Property, applied modifiers and nested options.
    */
  def apply(
    ...
    new InputBinding[TextArea] {
      private val element = textarea(
        textareaModifiers,
        nestedInterceptor(new TextAreaModifier(value, debounce, onInputElementEvent)),
      ).render

      override def render: TextArea = element
    }

  private class TextAreaModifier(
