...
/** A class (typically a component) exposing callbacks events.
  *
  * Callbacks emitted by this utility interface are executed in the order of registration (restart counts as an
  * registration). Events can be defined via implementing [[ListenableEvent]] and setting the `EventType` appropriately.
  */
trait Listenable {
  ...
  /** Registers an event handler in the component.
    *
    * @param onEvent
    *   Partial function which handles component events.
    * @return
    *   [[Registration]] which allows you to manage the lifecycle of the listener.
    */
