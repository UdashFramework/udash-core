...
/** The Presenter should contain all business logic of a view: user interaction callbacks, server communication. It
  * should not call any methods of a View class. The View and the Presenter should communicate via Model properties.
  * When implementing Presenter, you should remember, that a handleState method can be called not only on view
  * initialization.
  *
  * @tparam S
...
trait Presenter[-S <: State] {
  ...
  /** This method will be called when relevant state needs to be resolved. It can be uses to get parameters from state
    * and use it to call eg. external API.
    *
    * @param state
...
trait ViewFactory[S <: State] {
  ...
  /** Factory method which should return ready to used instance of [[io.udash.core.Presenter]] and
    * [[io.udash.core.View]].
    *
    * @return
    *   pair of presenter and view for state S
    */
  def create(): (View, Presenter[S])
}

/** Abstract view which should be used in order to implement View for [[io.udash.core.ViewFactory]]. The View
  * implementation usually gets the model and the [[io.udash.core.Presenter]] as constructor arguments.
  */
trait View {
...
/** The implementation of this trait should be injected to [[io.udash.Application]]. It is used to map [[State]] to
  * [[ViewFactory]].
  */
