...
trait UrlLogging[S >: Null <: GState[S]] extends CrossLogging { app: Application[S] =>
  ...
  app.onStateChange(event =>
    Try(log(matchState(event.currentState).value, Try(matchState(event.oldState).value).toOption)).failed.foreach(t =>
      logger.warn("Logging url change failed: {}", t.getMessage)
    )
  )
}
