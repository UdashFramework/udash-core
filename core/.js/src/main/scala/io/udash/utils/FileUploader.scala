...
class FileUploader(url: Url) {
  ...
  def uploadFile(
    fieldName: String,
    file: File,
    extraData: Map[String, String | Blob] = Map.empty,
    additionalRequestHeaders: Map[RequestName, RequestValue] = Map.empty,
  ...
  def upload(
    ...
    extraData.foreach { case (key, value) =>
      (value: Any) match {
