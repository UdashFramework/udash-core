...
class SeqAsValuePatchingModifier[T, E <: ReadableProperty[T]]
                                (property: ReadableSeqProperty[T, E],
                                 ...
                                 elementsUpdater: (Patch[E], Seq[Node], Binding => Binding) => Any)
  extends Binding {

  def this(property: ReadableSeqProperty[T, E], initBuilder: Seq[E] => Seq[Node], elementsUpdater: (Patch[E], Seq[Node]) => Any) = {
    this(property, (s, _) => initBuilder(s), (s, el, _) => elementsUpdater(s, el))
  }

  override def applyTo(t: Element): Unit = {
    val elements: Seq[Node] = initBuilder(property.elemProperties, nestedInterceptor)
