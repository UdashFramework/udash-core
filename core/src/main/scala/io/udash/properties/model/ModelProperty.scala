...
trait ModelProperty[A] extends AbstractProperty[A] with ModelPropertyMacroApi[A] {
  ...
  /** Returns child DirectSeqProperty[B] of any Seq-like field. Note that due to SeqProperty mutable nature, there may
    * be performance overhead while calling subSeq on fields of type more specific than scala.collection.Seq, e.g.
    * scala.collection.immutable.List or scala.collection.immutable.Seq
    */
  def subSeq[B, SeqTpe[T] <: BSeq[T]](
    f: A => SeqTpe[B]
