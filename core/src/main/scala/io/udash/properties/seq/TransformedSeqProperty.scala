...
private[properties] class TransformedReadableSeqProperty[
  A,
  B,
  ElemType <: ReadableProperty[B],
  OrigType <: ReadableProperty[A],
](
  override protected val origin: ReadableSeqProperty[A, OrigType],
  transformer: A => B,
  ...
  override protected def transformElements(elemProperties: BSeq[OrigType]): BSeq[ElemType] =
    elemProperties.map(transformElement)
