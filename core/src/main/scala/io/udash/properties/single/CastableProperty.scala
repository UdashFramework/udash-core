...
/** Property which can be casted. <br/> <b>NOTE:</b> Those casts are checked in compilation time: "it compiles" == "it
  * works".
  */
trait CastableReadableProperty[A] extends ReadableProperty[A] {
  ...
  def asSeq[B](implicit sev: A <:< BSeq[B], ev: SeqPropertyCreator[B, BSeq])
    : ReadableSeqProperty[B, CastableReadableProperty[B]] =
    this.asInstanceOf[ReadableSeqProperty[B, CastableReadableProperty[B]]]
}

/** Property which can be casted. <br/> <b>NOTE:</b> Those casts are checked in compilation time: "it compiles" == "it
  * works", as long as the property creators were not explicitly passed and relevant implicit scope is equal for
  * property creation and cast.
  */
    ...
    this match {
      ...
      case _ =>
        throw new IllegalStateException(
          "Property was created without provided ModelPropertyCreator in scope. " +
            "Make sure it is uniformly available, e.g. in the companion object of the model class."
        )
    }

  /** Safely casts `DirectProperty[Seq[A]]` to `DirectSeqProperty[A]` */
  override def asSeq[B](implicit sev: A <:< BSeq[B], ev: SeqPropertyCreator[B, BSeq])
    : SeqProperty[B, CastableProperty[B]] =
    (this: Any) match {
      case sp: SeqProperty[_, _] => sp.asInstanceOf[SeqProperty[B, CastableProperty[B]]]
      case _ =>
        throw new IllegalStateException(
          "Property was created without provided SeqPropertyCreator in scope. " +
