...
trait Property[A] extends ReadableProperty[A] {
  ...
  /** Creates Property[B] linked to `this`. Changes will be bidirectionally synchronized between `this` and new
    * property.
    *
    * @param transformer
  ...
  /** Bidirectionally synchronizes Property[B] with `this`. The transformed value is synchronized from `this` to
    * Property[B] on initialization.
    *
    * @param p
...
private[properties] trait AbstractProperty[A] extends AbstractReadableProperty[A] with Property[A] {
  ...
  override def bitransformToSeq[B: PropertyCreator](transformer: A => BSeq[B])(revert: BSeq[B] => A)
    : SeqProperty[B, Property[B]] =
