...
/** Base trait for all stylesheets.
  *
  * Example: <pre> object ExampleStylesheet extends CssBase { import dsl._
  *
  * val s = style(...) } </pre>
  */
trait CssBase {
  class Dsl(val elementsBuffer: mutable.ArrayBuffer[CssStyle]) extends DslBase {
    ...
    /** Creates CSS style which will not be rendered. <br/><br/>
      *
      * Example: <pre> def utils(x: Int): CssStyle = mixin( margin(x px, auto), textAlign.left, cursor.pointer ) </pre>
      */
    ...
    /** Creates CSS style with auto-generated name. <br/><br/>
      *
      * Example: <pre> val example: CssStyle = style( utils(12), // mixin usage
      *
      * &.hover( cursor.zoomIn ),
      *
      * media.not.handheld.landscape.maxWidth(640 px)( width(400 px) ) ) </pre>
      */
    ...
    /** Creates CSS keyframes animation with auto-generated name. <br/><br/>
      *
      * Example: <pre> val animation = keyframes( 0d -> keyframe(height(10 px)), 20d -> keyframe(height(50 px)), 100d ->
      * keyframe(height(200 px)) ) </pre>
      */
    ...
    /** Creates CSS fontface with auto-generated name. <br/><br/>
      *
      * Example: <pre> val ff = fontFace( _.src("url(font.woff)") .fontStretch.expanded .fontStyle.italic
      * .unicodeRange(0, 5) ) </pre>
      */
      ...
      case CssKeyframes(className, steps) =>
        ...
        renderer.apply(
          Vector(
            CssEntry.Keyframes(
              kfs.name,
              ListMap[KeyframeSelector, StyleStream]() ++ kfs.frames.iterator.map { case (selector, style) =>
