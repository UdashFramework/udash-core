...
object StylesheetExample extends CssBase {
  ...
  val ff = namedFontFace("myFont", _.src("url(font.woff)").fontStretch.expanded.fontStyle.italic.unicodeRange(0, 5))

  val kf1 = style(height(100.px), width(30.px))

  val kf2 = keyframe(height(150.px), width(30.px))
  ...
  val animation = keyframes(
    ...
    50d -> kf2,
    75d -> kf1,
    100d -> keyframe(height(200.px), width(60.px)),
  )
  ...
  val indent: Int => CssStyle = (0 to 3).map { i =>
