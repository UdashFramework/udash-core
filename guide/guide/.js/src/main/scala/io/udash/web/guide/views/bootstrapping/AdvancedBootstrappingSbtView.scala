...
case object AdvancedBootstrappingSbtViewFactory
  extends StaticViewFactory[AdvancedBootstrappingSbtState.type](() => new AdvancedBootstrappingSbtView)
...
final class AdvancedBootstrappingSbtView extends View with CssView {
  ...
  override def getTemplate: Modifier =
    div(
      ...
      p(
        "You can see the content of the build templates below. They're also available on ",
        a(href := UdashG8Repo, target := "_blank")("udash.g8"),
        " repository.",
      ),
      h3("build.sbt"),
      ...
      p(
        ...
        " as webserver nor to use ",
        a(href := BootstrapHomepage, target := "_blank")("Twitter bootstrap"),
        " for frontend components. Anyway it is recommended to use ",
        "Jetty for an easy start, since you can embed it in your code and deploy anywhere easily.",
      ),
      h2("What's next?"),
      p(
        "sbt configuration is ready, now it is time to prepare ",
        a(href := BootstrappingRpcState.url)("RPC interfaces"),
        " in the ",
        b("shared"),
        " module.",
      ),
