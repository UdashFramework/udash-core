...
case object FrontendBindingsViewFactory
  extends StaticViewFactory[FrontendBindingsState.type](() => new FrontendBindingsView)
...
class FrontendBindingsView extends View with CssView {
  ...
  override def getTemplate: Modifier = div(
    ...
    ul(GuideStyles.defaultList)(
      li(
        i("bind"),
        " - the simplest way to bind a property to a template, it uses the ",
        i(".toString"),
        " method to get the string which should be displayed.",
      ),
      li(
        i("produce"),
        " - similar to ",
        i("bind"),
        ", but takes a builder method which is called on every change of the property - its result is inserted into DOM.",
      ),
    ...
    p(
      "As you can see the ",
      i("bind"),
      " method automatically updates displayed name on every change of the property value.",
    ),
    h3("produce"),
    produceSnippet,
    produceDemo,
    p(
      "The above example presents two variants of the ",
      i("produce"),
      " method. This is very similar to the ",
      i("bind"),
    ...
    repeatDemo,
    p(
      "Notice that the version of ",
      i("produce"),
      " for ",
      i("SeqProperty"),
      ", ",
      "redraws the whole sequence every time - it is ok when the sequence is small. The ",
      i("repeat"),
      " method updates only changed elements of the sequence. To make it easier to notice, every added element is highlighted. ",
    ),
    h3("showIf"),
    p(
      "This binding method takes two arguments: ",
      i("Property[Boolean]"),
      " and ",
      i("Seq[dom.Element]"),
      ". ",
      "If value of the property is ",
      i("true"),
      " it shows all passed elements and hides them otherwise.",
    ),
    showIfSnippet,
    showIfDemo,
    h3("Attribute bindings"),
    p(
      "Udash provides extension methods on Scalatags ",
      i("Attr"),
      " and ",
      i("AttrPair"),
      ". ",
      i("Attr.bind(ReadableProperty[String])"),
      " synchronises attribute value with the property. ",
      i("AttrPair.attrIf(ReadableProperty[Boolean])"),
      " adds attribute if the property value is ",
      i("true"),
    ...
    p(
      "When you change ",
      i("p"),
      " value, the builder creates a new element with a new binding inside, but unfortunately ",
    ...
    p(
      ...
      " and ",
      i("repeatWithNested"),
      ". These bindings provide not only the property value, ",
    ...
    p(
      "Take a look at the ",
      a(href := FrontendFormsState.url)("Two-way Forms Binding"),
      " chapter to read about binding properties to HTML forms.",
    ),
  )
}
