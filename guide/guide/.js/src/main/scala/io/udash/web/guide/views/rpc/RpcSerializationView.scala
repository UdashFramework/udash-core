...
case object RpcSerializationViewFactory
  extends StaticViewFactory[RpcSerializationState.type](() => new RpcSerializationView)
...
class RpcSerializationView extends View with CssView {
  ...
  override def getTemplate: Modifier = div(
    ...
    p(
      ...
      a(href := References.AvScalaCommonsGitHub, target := "_blank")("AVSystem Scala Commons"),
      " library which supports serialization",
    ...
    p(
      ...
      " mechanism from the ",
      a(href := References.AvScalaCommonsGitHub, target := "_blank")("AVSystem Scala Commons"),
      " library for the data serialization. It provides ",
    ...
    ul(GuideStyles.defaultList)(
      ...
      li(
        "class or trait whose companion object has a pair of case-class-like ",
        i("apply"),
        " and ",
        i("unapply"),
        " methods and every parameter type of ",
        i("apply"),
        " method has its own ",
        i("GenCodec"),
        ",",
      ...
      li(
        "sealed hierarchy in which every non-abstract subclass either has its own ",
        i("GenCodec"),
        " or it can be automatically materialized with the same mechanism.",
      ),
    ),
    p(
      "For aformentioned types you can simply create companion object extending ",
      i("HasGenCodec[T]"),
      " class or ",
      "materialize codec with ",
      i("GenCodec.materialize[T]"),
      ".",
    ),
    ...
    p(
      "Compilation of this code rises an error: No ",
      i("GenCodec"),
      " found for ",
      i("GenCodecServerRPC.DemoClass"),
      ".",
    ),
    p(
      "If ",
      i("GenCodec"),
      " cannot be materialized automatically, you have to write it by yourself. ",
      "Method ",
      i("write"),
      " gets two arguments. ",
      i("Output"),
      " allows you to write the basic Scala types and data structures, ",
      "you should use it to create representation of object provided as second argument. Method ",
      i("read"),
      " should convert ",
      "provided ",
      i("Input"),
      " which allows you to read the basic Scala types and data structures to an object. Take a look at the example below. ",
    ),
    CodeBlock(
      """object DemoClass {
    ...
    p(
      "The ",
      i("GenCodec"),
      "s mechanism provides abstraction layer over the serialization to the raw string which is sent via websocket. ",
      "By default the Udash framework uses the ",
      i("GenCodec"),
      "s representation of data to serialize it to the JSON.",
    ),
    p(
      "Usually you do not need to worry about serialization from ",
      i("GenCodec"),
      " to the raw string, unless you want to change serialization mechanism.",
    ),
    h3("Custom serialization"),
    p(
      "It is possible to change the serialization from JSON to anything else. Instead of using ",
      i("DefaultUdashRPCFramework"),
    ...
    p(
      ...
      ul(GuideStyles.defaultList)(
        li(
          ...
          " - ",
          i("GenCodec"),
          " needs to be converted to this type and then this type needs to be converted into the string. Usually it is some kind of AST.",
        ),
