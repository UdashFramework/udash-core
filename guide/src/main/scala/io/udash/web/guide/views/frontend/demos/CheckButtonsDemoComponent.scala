  ...
  def checkboxes() = div(BootstrapStyles.inputGroup, GuideStyles.blockOnMobile)(
    ...
    div(BootstrapStyles.inputGroupAddon, GuideStyles.blockOnMobile)(
      CheckButtons(
        ...
        (els: Seq[(Input, String)]) => span(els.map { case (i: Input, l: String) => label(BootstrapStyles.checkboxInline, "data-label".attr := l)(i, l) })
      )
    ),
    div(BootstrapStyles.inputGroupAddon, GuideStyles.blockOnMobile)(span(cls := "check-buttons-demo-fruits")(bind(favoriteFruits)))
  )
}
