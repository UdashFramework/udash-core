...
class ResourceBundlesTranslationTemplatesProviderTest extends UdashRpcBackendTest {
  ...
  val bundles = Seq("en", "pl")
    .map(lang =>
      Lang(lang) ->
        testBundlesNames.map(name => ju.ResourceBundle.getBundle(name, new ju.Locale.Builder().setLanguage(lang).build()))
    )
    .toMap
  ...
  "ResourceBundlesTranslationTemplatesProvider" should {
    ...
    "throw an exception when mixed placeholders occurs" in {
      val mixedProvider = new ResourceBundlesTranslationTemplatesProvider(
        Map(Lang("en") -> Seq(ju.ResourceBundle.getBundle("mixed", new ju.Locale.Builder().setLanguage("en").build())))
      )
