...
private[udash] class AllValuesMacro(override val c: blackbox.Context) extends AbstractMacroCommons(c) {
  ...
  def ofType[V: WeakTypeTag](obj: Tree): Tree = {
    ...
    val names = accessibleMembers(obj.tpe).iterator
      .filter { s =>
        (s.isMethod || s.isModule) && s.typeSignature.paramLists.iterator.flatten.isEmpty &&
        s.typeSignature.typeParams.isEmpty && s.typeSignatureIn(obj.tpe).finalResultType <:< valueType
      }
