...
object Dependencies {
  ...
  val compilerPlugins = Def.setting(
    Seq(
      "com.avsystem.commons" %% "commons-analyzer" % avsCommonsVersion
    ).map(compilerPlugin)
  )

  val commonTestDeps = Def.setting(
    Seq(
      "org.scalatest" %%% "scalatest" % scalatestVersion
    ).map(_ % Test)
  )

  val commonJsTestDeps = Def.setting(
    Seq(
      "org.scala-js" %%% "scalajs-fake-insecure-java-securerandom" %
        scalaJsSecureRandomVersion // ScalaTest uses SecureRandom
    ).map(_ % Test)
  )

  val macroDeps = Def.setting(
    Seq(
  ...
  val utilsCrossDeps = Def.setting(
    Seq(
      "com.avsystem.commons" %%% "commons-core" % avsCommonsVersion
    )
  )

  val utilsJvmDeps = Def.setting(
    utilsCrossDeps.value ++ Seq(
      "com.typesafe.scala-logging" %% "scala-logging" % scalaLoggingVersion
    )
  )

  val utilsSjsDeps = Def.setting(
    utilsCrossDeps.value ++ Seq(
      "org.scala-js" %%% "scalajs-dom" % scalaJsDomVersion
    )
  )

  private val coreCrossDeps = Def.setting(
    Seq(
  ...
  private val restCrossDeps = Def.setting(
    Seq(
      "com.avsystem.commons" %%% "commons-core" % avsCommonsVersion,
  ...
  val restJvmDeps = Def.setting(
    restCrossDeps.value ++ Seq(
      "javax.servlet" % "javax.servlet-api" % servletVersion,
      "com.typesafe.scala-logging" %% "scala-logging" % scalaLoggingVersion,
  ...
  val restJettyDeps = Def.setting(
    Seq(
      "org.eclipse.jetty" % "jetty-client" % jettyVersion
    )
  )

  private val cssCrossDeps = Def.setting(
    Seq(
  ...
  val cssSjsDeps = Def.setting(
    cssCrossDeps.value ++ Seq(
      "com.lihaoyi" %%% "scalatags" % scalaTagsVersion
    )
  )

  val bootstrap4SjsDeps = Def.setting(
    Seq(
  ...
  val bootstrap4JsDeps = Def.setting(
    Seq[JSModuleID](
      ...
      ("org.webjars" % "tempusdominus-bootstrap-4" % bootstrap4DatepickerVersion / "js/tempusdominus-bootstrap-4.js")
        .minified("js/tempusdominus-bootstrap-4.min.js")
        .dependsOn(bootstrap4Resource, momentResource),
    )
  )

  val benchmarksSjsDeps = Def.setting(
    Seq(
  ...
  val guideJsDeps = Def.setting(
    Seq[JSModuleID](
      ProvidedJS / "prism.js"
    )
  )

  val guideFrontendDeps = Def.setting(
    Seq(
      "org.webjars" % "font-awesome" % fontAwesomeVersion
    )
  )

  val homepageJsDeps = Def.setting(
    Seq[JSModuleID](
      ProvidedJS / "prism.js"
    )
  )
