...
abstract class AbstractRestDataCompanion[Implicits, T](
  implicits: Implicits
...
/** Base class for companion objects of ADTs (case classes, objects, sealed hierarchies) which are used as parameter or
  * result types in REST API traits. Automatically provides instances of `GenCodec` and
  ...
  *   }}}
  */
abstract class RestDataCompanion[T](implicit instances: MacroInstances[DefaultRestImplicits, CodecWithStructure[T]])
  extends AbstractRestDataCompanion[DefaultRestImplicits, T](DefaultRestImplicits)

/** A version of [[RestDataCompanion]] which injects additional implicits into macro materialization. Implicits are
  * imported from an object specified with type parameter `D`. It must be a singleton object type, i.e.
  * `SomeObject.type`.
  */
abstract class RestDataCompanionWithDeps[D, T](
  implicit instances: MacroInstances[(DefaultRestImplicits, D), CodecWithStructure[T]],
  deps: scala.ValueOf[D],
) extends AbstractRestDataCompanion[(DefaultRestImplicits, D), T]((DefaultRestImplicits, deps.value)) {
  @bincompat
  @nowarn("msg=deprecated")
  private[rest] def this(
    applyUnapplyProvider: misc.ValueOf[D],
    instances: MacroInstances[(DefaultRestImplicits, D), CodecWithStructure[T]],
  ) = this()(instances, applyUnapplyProvider.toScala)
}

/** These implicits must be specialized for every raw type (PlainValue, JsonValue, etc.) because it lifts their
  * priority. Unfortunately, controlling implicit priority is not pretty. Also, it's probably good that we explicitly
  * enable derivation only for REST-related raw types and not for all raw types - this avoids possible interference with
  * other features using RPC.
  *
...
trait RestDataWrapperImplicits[Wrapped, T] {
  ...
  implicit def streamedResponseAsRaw(implicit wrappedAsRaw: AsRaw[StreamedRestResponse, Wrapped])
    : AsRaw[StreamedRestResponse, T] =
    AsRaw.fromTransparentWrapping

  implicit def streamedResponseAsReal(implicit wrappedAsRaw: AsReal[StreamedRestResponse, Wrapped])
    : AsReal[StreamedRestResponse, T] =
    AsReal.fromTransparentWrapping
...
/** Base class for companion objects of wrappers over other data types (i.e. case classes with single field). This
  * companion ensures instances of all the REST typeclasses (serialization, schema, etc.) for wrapping type assuming
  * that these instances are available for the wrapped type.
  *
  ...
  *   }}}
  */
abstract class RestDataWrapperCompanion[Wrapped, T](
  implicit instances: MacroInstances[DefaultRestImplicits, () => NameAndAdjusters[T]]
) extends TransparentWrapperCompanion[Wrapped, T]
    with RestDataWrapperImplicits[Wrapped, T] {
...
/** Base class for companion objects of enum types [[ValueEnum]] which are used as parameter or result types in REST API
  * traits. Automatically provides instance of [[io.udash.rest.openapi.RestSchema RestSchema]] that will be <b>named</b>
  * and used as a reference in OpenAPI model.
  * {{{
  ...
  * }}}
  *
  * [[io.udash.rest.openapi.RestSchema RestSchema]] instance generated by default for enums is inlined, see
  * [[RestSchema.namedEnumSchema]].
  */
abstract class RestValueEnumCompanion[E <: ValueEnum](
