...
abstract class RestClientApiCompanion[Implicits, Real](
  protected val implicits: Implicits
...
/** @see [[io.udash.rest.RestApiCompanion RestApiCompanion]] */
abstract class RestServerApiCompanion[Implicits, Real](
  protected val implicits: Implicits
  ...
  /** Maintained for backward compatibility with non-streaming clients. Converts the real API implementation into a
    * request handler without streaming support.
    */
...
/** @see [[io.udash.rest.RestApiCompanion RestApiCompanion]] */
abstract class RestServerOpenApiCompanion[Implicits, Real](
  protected val implicits: Implicits
...
/** Base class for REST trait companions. Reduces boilerplate needed in order to define appropriate instances of
  * `AsRawReal` and `RestMetadata` for given trait. The `Implicits` type parameter lets you inject additional implicits
  * into macro materialization of these instances, e.g. [[io.udash.rest.DefaultRestImplicits DefaultRestImplicits]].
  * Usually, for even less boilerplate, this base class is extended by yet another abstract class which fixes the
  * `Implicits` type, e.g. [[io.udash.rest.DefaultRestApiCompanion DefaultRestApiCompanion]].
  */
abstract class RestApiCompanion[Implicits, Real](
  protected val implicits: Implicits
...
/** @see [[io.udash.rest.RestApiCompanion RestApiCompanion]] */
abstract class RestOpenApiCompanion[Implicits, Real](
  protected val implicits: Implicits
...
abstract class DefaultPolyRestApiCompanion[T[_[_]]](
  implicit instances: MacroInstances[GenCodecRestImplicits, PolyRestApiFullInstances[T]]
) {
...
/** Like [[RestServerApiCompanion]] but the `Real` type is supposed to be a class with REST methods already implemented -
  * as opposed to a trait with abstract methods that needs separate implementation. All public methods of this class -
  * abstract or concrete - will be interpreted as REST methods (unless annotated with
  * [[com.avsystem.commons.meta.ignore ignore]]).
  */
abstract class RestServerApiImplCompanion[Implicits, Real](
  protected val implicits: Implicits
...
/** Like [[RestServerApiImplCompanion]] but additionally materializes `OpenApiMetadata` for the class.
  */
abstract class RestServerOpenApiImplCompanion[Implicits, Real](
  protected val implicits: Implicits
