...
trait FutureRestImplicits {
  ...
  /** This `Scheduler` is used only on client-side, effectively only to translate between
    * [[RestRequest]]/[[RestResponse]] and native representations of requests and responses for the HTTP client being
    * used (e.g. Jetty). Despite that, it is recommended to override this method and provide a customized `Scheduler`
...
trait DefaultRestImplicits extends FutureRestImplicits with GenCodecRestImplicits {
  @implicitNotFound(
    "${T} is not a valid server REST API trait, does its companion extend " +
  ...
  @implicitNotFound(
    "${T} is not a valid REST API trait, does it have a companion object which extends " +
  ...
  @implicitNotFound(
    "${T} is not a valid REST API trait for OpenAPI generation, does it have a companion object " +
  ...
  @implicitNotFound(
    "${T} is not a valid client REST API trait, does it have a companion object that extends " +
  ...
  @implicitNotFound(
    "RestSchema for ${T} not found. To provide it for case classes and sealed hierarchies " +
