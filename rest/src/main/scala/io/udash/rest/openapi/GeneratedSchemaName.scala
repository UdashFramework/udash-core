...
/** Can be used on case class or sealed hierarchy root to instruct OpenAPI schema derivation mechanism to use particular
  * value as [[RestSchema]] name instead of just using class name.
  *
  * Takes precedence over [[com.avsystem.commons.serialization.name]], but unlike
  * [[com.avsystem.commons.serialization.name]] does <b>NOT</b> change type discriminator value when the class is a part
  * of a sealed hierarchy. This is useful to resolve schema name conflicts with other classes without changing
  * serialized representation.
  */
class schemaName(val name: String) extends StaticAnnotation with NotInheritedFromSealedTypes

/** A metadata typeclass that you can use to control names of [[RestSchema]]s macro-materialized for ADTs (case classes
  * & sealed hierarchies).
  *
  * [[GeneratedSchemaName]] is captured by [[RestStructure]] which is then used to construct a [[RestSchema]] instance.
  *
  * Customizing this implicit is mostly useful when used collectively, i.e. you can derive schema name as fully
  * qualified class name from a `ClassTag`, in a generic `implicit def`. Example:
  *
  * {{{
  ...
  *
  * @param name
  *   the schema name or `Opt.Empty` for an unnamed (inline) schema
  */
...
object GeneratedSchemaName extends GeneratedSchemaNameLowPrio {
  ...
  /** Creates an empty [[GeneratedSchemaName]], which indicates that a [[RestSchema]] based on [[RestStructure]] should
    * be anonymous.
    */
  def none[T]: GeneratedSchemaName[T] = GeneratedSchemaName(Opt.Empty)

  /** Creates a [[GeneratedSchemaName]] with given name, which specifies the name of [[RestSchema]] based on
    * [[RestStructure]] for given type.
    */
