...
trait RestSchema[T] { self =>
  ...
  /** Creates a [[Schema]] object or external schema reference. May use [[SchemaResolver]] to resolve any dependent
    * `RestSchema` instances. This method should be called directly only by [[SchemaResolver]].
    */
  ...
  /** Optional name of the schema. When `RestSchema` is named, schema created by [[createSchema]] will be registered
    * under that name in [[SchemaRegistry]] and ultimately included into [[Components]] of the [[OpenApi]] document. All
    * direct usages of the schema in OpenAPI document will be replaced by a reference to the registered schema, i.e.
...
object RestSchema {
          ...
          List(
            RefOr(
              Schema(
                ...
                properties = Map("Left" -> resolver.resolve(RestSchema[A])),
                required = List("Left"),
              )
            ),
            RefOr(
              Schema(
                `type` = DataType.Object,
                properties = Map("Right" -> resolver.resolve(RestSchema[B])),
                required = List("Right"),
              )
            ),
          )
  ...
  implicit def namedEnumMapSchema[M[X, Y] <: BMap[X, Y], K <: NamedEnum: NamedEnumCompanion, V: RestSchema]
    : RestSchema[M[K, V]] =
  ...
  implicit def namedEnumJMapSchema[M[X, Y] <: JMap[X, Y], K <: NamedEnum: NamedEnumCompanion, V: RestSchema]
    : RestSchema[M[K, V]] =
    RestSchema[V].map(Schema.enumMapOf(enumValues[K], _))
...
/** Intermediate typeclass which serves as basis for [[RestResponses]] and [[RestRequestBody]]. [[RestMediaTypes]] is
  * derived by default from [[RestSchema]]. It should be defined manually for every type which has custom serialization
  * to [[io.udash.rest.raw.HttpBody HttpBody]] defined so that generated OpenAPI properly reflects that custom
...
trait RestMediaTypes[T] {
  ...
  /** @param schemaTransform
    *   Should be used if [[RestMediaTypes]] is being built based on [[RestSchema]] for the same type. The
    *   transformation may adjust the schema and give it a different name. This transformation is usually used when
    *   there's a type that wraps another type and wants to reuse [[RestMediaTypes]] of the wrapped type but also
    *   introduces some schema modifications. See [[io.udash.rest.RestDataWrapperCompanion]].
    */
...
/** Typeclass which defines how an OpenAPI [[Responses]] Object will look like for a given type. By default,
  * [[RestResponses]] is derived based on [[RestMediaTypes]] for that type which is itself derived by default from
  * [[RestSchema]] for that type. It should be defined manually for every type which has custom serialization to
  * [[io.udash.rest.raw.RestResponse RestResponse]] defined so that generated OpenAPI properly reflects that custom
  * serialization format.
  */
...
trait RestResponses[T] {
  ...
  /** @param schemaTransform
    *   Should be used if [[RestResponses]] is being built based on [[RestSchema]] for the same type. The transformation
    *   may adjust the schema and give it a different name. This transformation is usually used when there's a type that
    *   wraps another type and wants to reuse [[RestResponses]] of the wrapped type but also introduces some schema
...
object RestResponses {
  ...
  implicit val UnitResponses: RestResponses[Unit] =
    (_: SchemaResolver, _: RestSchema[_] => RestSchema[_]) =>
  ...
  implicit def notFound[T](implicit forMediaTypes: ImplicitNotFound[RestMediaTypes[T]])
    : ImplicitNotFound[RestResponses[T]] =
    ImplicitNotFound()
}

/** Just like [[io.udash.rest.openapi.RestResponses RestResponses]],
  * [[io.udash.rest.openapi.RestResultType RestResultType]] is a typeclass that defines how an OpenAPI Responses Object
  * will look like for an HTTP method which returns given type. The difference between
  * [[io.udash.rest.openapi.RestResultType RestResultType]] and [[io.udash.rest.openapi.RestResponses RestResponses]] is
  * that [[io.udash.rest.openapi.RestResultType RestResultType]] is defined for full result type which usually is some
  * kind of asynchronous wrapper over actual result type (e.g. `Future`). In such situation,
  * [[io.udash.rest.openapi.RestResponses RestResponses]] must be provided for `T` while
  * [[io.udash.rest.openapi.RestResultType RestResultType]] is provided for `Future[T]` (or whatever async wrapper is
  * used), based on the [[io.udash.rest.openapi.RestResponses RestResponses]] instance of `T`. You can see an example of
  * this in [[io.udash.rest.FutureRestImplicits FutureRestImplicits]].
  *
  * [[io.udash.rest.openapi.RestResultType RestResultType]] for
  * [[io.udash.rest.openapi.OpenApiMetadata OpenApiMetadata]] is analogous to
  * [[io.udash.rest.raw.HttpResponseType HttpResponseType]] for [[io.udash.rest.raw.RestMetadata RestMetadata]].
  */
...
/** Typeclass which defines how OpenAPI [[RequestBody]] Object will look like for a Given type when that type is used as
  * a type of [[io.udash.rest.Body Body]] parameter of a [[io.udash.rest.CustomBody CustomBody]] method. By default,
  * [[RestRequestBody]] is derived from [[RestMediaTypes]] which by itself is derived by default from [[RestSchema]].
  */
@implicitNotFound("RestRequestBody instance for ${T} not found")
trait RestRequestBody[T] {

  /** @param schemaTransform
    *   Should be used if [[RestRequestBody]] is being built based on [[RestSchema]] for the same type. The
    *   transformation may adjust the schema and give it a different name.
    */
...
object RestRequestBody {
  ...
  implicit val UnitRequestBody: RestRequestBody[Unit] = (_: SchemaResolver, _: RestSchema[_] => RestSchema[_]) =>
    Opt.Empty
  ...
  implicit def notFound[T](implicit forMediaTypes: ImplicitNotFound[RestMediaTypes[T]])
    : ImplicitNotFound[RestRequestBody[T]] =
    ImplicitNotFound()
}

trait SchemaResolver {

  /** Resolves a [[RestSchema]] instance into an actual [[Schema]] object or reference. If the schema is unnamed then
    * this method will simply return the same value as [[RestSchema.createSchema]]. If the schema is named, it may be
    * internally registered under its name and a
...
/** An implementation of [[SchemaResolver]] which registers named [[RestSchema]]s and replaces them with a
  * [[https://github.com/OAI/OpenAPI-Specification/blob/master/versions/3.0.2.md#referenceObject Reference Object]]. All
  * the registered schemas can then be extracted and listed in the [[Components]] object.
  */
final class SchemaRegistry(
  ...
  private[this] val resolving = new MHashSet[String]
  private[this] val registry = new MHashMap[String, MListBuffer[Entry]].setup(_ ++= initial.iterator.map {
    case (n, s) => (n, MListBuffer[Entry](Entry(Opt.Empty, s)))
  })
    ...
    case Opt(name) =>
      if (!resolving.contains(name)) { // handling recursive schemas
        ...
        if (!entries.exists(_.source.contains(restSchema))) {
          resolving += name
          val newSchema =
            try restSchema.createSchema(this)
            finally {
              resolving -= name
            }
