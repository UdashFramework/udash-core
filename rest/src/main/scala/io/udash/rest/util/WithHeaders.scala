...
/** Wrapper over a type which adds some arbitrary header values to it. Can be used as response type of REST HTTP
  * methods. Since these additional headers may be arbitrary, their presence will not be reflected in the OpenAPI
  * documentation. If you want to include this information into OpenAPI definition for method that returns
  * `WithHeaders`, you may use [[io.udash.rest.adjustResponse adjustResponse]] on it.
  */
