import {encodePath, encodeQuery, RestBody, RestRequest, RestResponse} from "./raw"

export function handleUsingFetch(baseUrl: string, request: RestRequest): Promise<RestResponse> {
    if (!baseUrl.endsWith("/")) {
        baseUrl = baseUrl + "/"
    }
