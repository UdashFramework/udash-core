...
/** Template of a servlet handling files upload. It takes files from the request and passes data to the `handleFile`
  * method.
  * @param fileFields
...
abstract class FileUploadServlet(fileFields: Set[String]) extends HttpServlet {
  ...
  override protected def doPost(request: HttpServletRequest, response: HttpServletResponse): Unit = {
    request.getParts.asScala.filter(part => fileFields.contains(part.getName)).foreach { filePart =>
