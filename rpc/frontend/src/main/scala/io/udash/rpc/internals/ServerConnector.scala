...
abstract class AtmosphereServerConnector[RPCRequest](
    ...
    val atmRequest: AtmosphereRequest = {
      if (websocketSupport)
        createRequestObject(
          ...
          onOpen = _ => ready(ConnectionStatus.Open),
          onReopen = _ => ready(ConnectionStatus.Open),
          onReconnect = (_, _) => {
          ...
          onError = _ => ready(ConnectionStatus.Closed),
          onClose = _ => ready(ConnectionStatus.Closed),
          onClientTimeout = _ => ready(ConnectionStatus.Closed)
        )
      else {
  ...
  private def createRequestObject(
    transport: Transport, reconnectInterval: Int,
    onOpen: js.Function1[AtmosphereResponse, Any] = _ => {},
    onReopen: js.Function1[AtmosphereResponse, Any] = _ => {},
    onClose: js.Function1[AtmosphereResponse, Any] = _ => {},
    onError: js.Function1[AtmosphereResponse, Any] = _ => {},
    onReconnect: js.Function2[AtmosphereRequest, AtmosphereResponse, Any] = (_, _) => {},
    onClientTimeout: js.Function1[AtmosphereResponse, Any] = _ => {},
    onTransportFailure: js.Function2[String, AtmosphereRequest, Any] = (_, _) => {}
  ): AtmosphereRequest = {
    AtmosphereRequest(
      url = serverUrl,
