...
/** Helper class for callbacks handling. The callbacks are executed in order of registration. Each callback is executed
  * once, it swallows exceptions thrown in callbacks.
  */
final class CallbacksHandler[ItemType] extends CrossLogging {
  ...
  /** Registers callback and returns `Registration`. Registration operations don't preserve callbacks order.
    */
